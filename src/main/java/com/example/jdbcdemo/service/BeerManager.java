...
public class BeerManager implements IBeerManager {
	...
	private String createTableBeer = "CREATE TABLE Beer(id int GENERATED BY DEFAULT AS IDENTITY, name varchar(20) UNIQUE, type varchar(30), percentOfAlcohol double, price double)";
	...
	public BeerManager() {
		try {
			...
			deleteBeerStmt = connection
					.prepareStatement("DELETE FROM Beer WHERE name=?");
	...
	public int deleteBeer(String name){
		int count = 0;
		try{
			deleteBeerStmt.setString(1,name);
	...
	public void addAllBeers(List<Beer> beers){
		try{
	...
	public void deleteAllBeers(List<String> beersNames){
		try{
			connection.setAutoCommit(false);
			for(String beerName: beersNames){
				deleteBeerStmt.setString(1,beerName);
				deleteBeerStmt.executeUpdate();
				if(beerName == "Test"){
